...
from medical.medical_config import MedicalConfig as Config
...
- For diagnosis queries: use diagnosis_code1d (ICD-10) OR diagnosis ILIKE '%term%' OR icd_remarks_d ILIKE '%term%'.
...
3. PostgreSQL syntax: EXTRACT(YEAR FROM TO_DATE('col', 'YYYY-MM-DD')), ILIKE (case-insensitive), LIMIT N.
...
6. For DIAGNOSIS queries, always try BOTH approaches:
   - diagnosis_code1d ILIKE 'X%' (ICD-10 code match)
   - OR icd_remarks_d ILIKE '%KEYWORD%' (free-text match)
...
9. For rates/percentages: CAST(SUM(CASE WHEN condition THEN 1 ELSE 0 END) AS numeric) * 100.0 / COUNT(*)
...
3. PostgreSQL syntax ONLY: EXTRACT(YEAR FROM TO_DATE('col', 'YYYY-MM-DD')), ILIKE (case-insensitive), LIMIT N.
...
6. FOR DIAGNOSIS QUERIES — use DUAL approach:
   WHERE (diagnosis_code1d ILIKE 'I10%' OR icd_remarks_d ILIKE '%HYPERTENSION%')
   This handles the 16% null rate in diagnosis_code1d.
7. FOR MORTALITY RATES:
   CAST(SUM(CASE WHEN is_death = 1 THEN 1 ELSE 0 END) AS numeric) * 100.0 / COUNT(*)
